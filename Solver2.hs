...
nextPuzzles p
  ...
  | otherwise =
    filter
      validColumns
      [setRowStrip p strip incompleteLoc | strip <- groupPossibilities]
  where
...
isValidGroup c s = isPossible && noRepeats && validSum
  where
    ...
    filledIn = entries == s
    validSum =
      (filledIn && sum squareValues == c) ||
      (not filledIn && sum squareValues < c)
      ...
      map
        (\x ->
           if x == Empty
             then filter
                    (`notElem` entryValues)
                    (possibilities newClue numEmpty)
             else [entryVal x])
        s
...
firstEmptyRowStrip p
  ...
  where
    ...
    findStart (x, y) =
      if x > 0 && isWritable (squareAt p (x - 1, y))
        then findStart (x - 1, y)
        else x
    findEnd :: (Int, Int) -> Int
    findEnd (x, y) =
      if x < (length (columns p)) - 1 && isWritable (squareAt p (x + 1, y))
        then findEnd (x + 1, y)
        else x
...
getRowStrip _ (_, _, 0) = []
getRowStrip p (x, y, l) =
  (squareAt p (x, y)) : (getRowStrip p (x + 1, y, l - 1))

setRowStrip :: Puzzle -> [Square] -> RowStripLoc -> Puzzle
setRowStrip p [] _ = p
setRowStrip p _ (_, _, 0) = p
setRowStrip p (s:ss) (x, y, l) =
  setRowStrip (setSquare p s (x, y)) ss (x + 1, y, l - 1)
...
intGroupPossibilities c s = map (\l -> changeWith 0 l s) noRepeatPossibilities
  where
    ...
    noRepeatPossibilities =
      filter (not . containsAny entryValues) emptyPossibilities
